...
class MyValidate {
  ...
  __initMethods() {
    ...
    that.methods = {
       ...
       */
      email(value) {
        return that.optional(value) || /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/.test(value);
      },
      /**
       * 验证手机格式
       */
       ...
       */
      noDigits(value) {
        return that.optional(value) || !/[0-9]/.test(value);
      },
      /**
       * 验证身份证号码
       */
   ...
   * @param {String} key 需要单独验证的key
   */
  checkForm(data, key = "") {
    this.__initData();
    if (key) {
      this.setView(key);
      this.checkParam(key, this.rules[key], data);
    } else {
